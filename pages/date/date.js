...
const conf = {
  ...
  getPirceInfo(year, month, day) {
    const self = _getCurrentPage();
    for (let i = 0; i < self.data.calendarPriceList.length; i++) {
      let priceItem = self.data.calendarPriceList[i];
      if (priceItem && priceItem.travelDate) {
        let priceDataArr = priceItem.travelDate.split("-");
        if (parseInt(priceDataArr[0]) == year && parseInt(priceDataArr[1]) == month && parseInt(priceDataArr[2]) == day) {
          return priceItem;
        }
      }
    }
  ...
  calculateDays(year, month, curDate) {
    ...
    for (let i = 1; i <= thisMonthDays; i++) {
      let priceInfo = conf.getPirceInfo(year, month, i);
      days.push({
        day: i,
  ...
  tapDayItem(e) {
    ...
    if (days[idx] && days[idx].priceInfo && this.selectDateCallBack) {
      this.selectDateCallBack.call(this, days[idx].priceInfo);
      this.setData({
...
export default (config = {}) => {
  ...
  self.setData({
    ...
    calendarPriceList: config.calendarPriceList || []
  });
  ...
  if (config.selectDateCallBack) {
    self.selectDateCallBack = config.selectDateCallBack.bind(self);
  }
};
